...
URLGUI = "https://cmsweb.cern.ch/dqm/offline/data/json/samples"
MATCH = '?match=/MinimumBias/Run2012.*-13Jul2012-v.*/DQM'
...
data = cjson.decode(buf.getvalue())
for type in data['samples']:
  print type['type']
  for run in type['items']:
    print run['run'], run['dataset']
